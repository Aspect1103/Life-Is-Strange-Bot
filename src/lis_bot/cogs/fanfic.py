...
# Custom
from lis_bot.config import ao3Password, ao3Username, serviceAccount
...
ignorePath = (
    rootDirectory.joinpath("resources").joinpath("files").joinpath("ignoreFics.txt")
)
...
class Fanfic(Cog):
    ...
    def __init__(self, bot: bridge.Bot) -> None:
        self.bot = bot
        self.session = AO3.Session(ao3Username, ao3Password)
        self.colour = Colour.green()
        self.ignore = list(open(ignorePath).readlines())
        self.quoteSearcher = None
        tempWorksheet = (
            gspread.service_account_from_dict(serviceAccount)
            .open("Life Is Strange Read Fanfictions")
            .worksheet("Life Is Strange Read Fanfictions")
            .get_all_values()[2:]
        )
    ...
    def quoteMaker(
        self, ficLink: str
    ) -> tuple[str, AO3.Work | None, str | None, str | None]:
        ...
        while quoteTries < 5:
            ...
            if 10 <= len(quote.split()) <= 170:
                return (
                    quote,
                    work,
                    self.listConverter(work.authors, True),
                    randomChapter.title,
                )
        # No quotes found
        return "", None, None, None
    ...
    def listConverter(self, lst: list[Any], author: bool = False) -> str:
        return (
            ", ".join(list(lst))
            if not author
            else ", ".join([author.username for author in lst])
        )

    # Function to create embeds
    def quoteEmbedCreater(
        self, quote: str, work: AO3.Work, authors: str, chapterName: str
    ) -> Embed:
    ...
    async def findLastQuote(
        ...
        async for message in ctx.channel.history(limit=100, before=before):
            ...
            if message.author.id == self.bot.user.id:
                try:
                    if (
                        message.embeds[0].author.name != Embed.Empty
                        and "https://archiveofourown.org" in message.embeds[0].url
                    ):
        ...
        return (
            None
            if repeats == 5
            else await self.findLastQuote(ctx, lastMessage.created_at, repeats)
        )
    ...
    @option(
        "includensfw",
        str,
        description="Include NSFW or not",
    ...
    async def quote(
        ...
        if includensfw.capitalize() == "Yes":
            tempArr = [
                item
                for item in self.worksheetArray
                if item[7] == "Yes" or item[7] == "?"
            ]
        else:
        ...
        while quote == "" and ficCount < 5:
            ficCount += 1
            quote, work, authors, chapterName = self.quoteMaker(
                tempArr.pop(random.randrange(len(tempArr)))[10]
            )
        if quote == "":
            await utils.commandDebugEmbed(ctx, "No valid quotes found")
        else:
            await ctx.respond(
                embed=self.quoteEmbedCreater(quote, work, authors, chapterName)
            )

    # nextquote command with a cooldown of 1 use every 45 seconds per guild
    @bridge.bridge_command(
        aliases=["nq"],
        description="Finds the last quote posted and picks another quote from the same story",
    )
    @commands.cooldown(1, utils.medium, commands.BucketType.guild)
    async def nextquote(
        self, ctx: bridge.BridgeApplicationContext | bridge.BridgeExtContext
    ) -> None:
        lastQuote = await self.findLastQuote(ctx)
        if lastQuote is None:
            await utils.commandDebugEmbed(
                ctx, f"Cannot find the last quote. Try running {ctx.prefix}quote"
            )
        else:
    ...
    @bridge.bridge_command(
        description="Finds the last quote posted and displays the metadata for that fic"
    )
    @commands.cooldown(1, utils.medium, commands.BucketType.guild)
    async def outline(
        self, ctx: bridge.BridgeApplicationContext | bridge.BridgeExtContext
        ...
        if lastQuote is None:
            await utils.commandDebugEmbed(
                ctx, "Cannot find the last quote. Try running /quote"
            )
        else:
            ...
            infoEmbed.set_footer(
                text=f"Words: {work.words}. Chapters: {work.nchapters}. Language: {work.language}"
            )
            # Display the embed
    ...
    @bridge.bridge_command(
        description="Finds the last quote posted and displays all works posted by that author"
    )
            ...
            if user.work_pages <= pageLimit:
                ...
                if len(works) > workLimit:
                    ...
                    for count, arr in enumerate(splitList):
                        ...
                        tempEmbed.set_footer(
                            text=f"Page {count+1} of {maxPage}. Fic Total: {len(works)}"
                        )
                        for work in arr:
                ...
                await utils.commandDebugEmbed(
                    ctx,
                    f"{authorName} has {user.works} works which is over the limit of {pageLimit * 20}",
                )
