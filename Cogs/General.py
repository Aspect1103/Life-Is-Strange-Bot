    ...
    async def guess(self, ctx, character=None):
        if str(self.gameManager.gameObj) == "Hangman":
            if ctx.author.id == self.gameManager.gameObj.user.id:
                ...
                await Utils.commandDebugEmbed(ctx.channel, True, f"Only {self.gameManager.gameObj.user.name}#{self.gameManager.gameObj.user.discriminator} can guess characters")
        else:
            await Utils.commandDebugEmbed(ctx.channel, True, f"Game is not currently running. Start it with {ctx.prefix}hangman start")
