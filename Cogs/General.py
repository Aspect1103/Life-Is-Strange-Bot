...
amaPath = os.path.join(rootDirectory, "TextFiles", "../TextFiles/AMA.txt")
...
class General(commands.Cog):
    ...
    def generalInit(self):
        ...
        # Create ama questions array
        temp = []
        with open(amaPath, "r", encoding="utf") as file:
        ...
        self.allowedIDsAMA = Utils.allowedIDs["ama"]
    ...
    async def question(self, ctx):
        if self.nextAMA == len(self.amaArray):
            # All questions done
            random.shuffle(self.amaArray)
            self.nextAMA = 0
        randomQuestion = self.amaArray[self.nextAMA]
        self.nextAMA += 1
        questionEmbed = Embed(title="AMA Quwestion", description=randomQuestion, colour=self.colour)
        questionEmbed.set_footer(text=f"{len(self.amaArray)} questions")
    ...
    async def cog_check(self, ctx):
        if str(ctx.command) == "trivia":
            ...
            return Utils.channelCheck(ctx, self.allowedIDsAMA)
    ...
    async def cog_command_error(self, ctx, error):
        if isinstance(error, commands.CheckFailure):
            if str(ctx.command) == "trivia":
                ...
                textChannelAllowed = [self.client.get_channel(channel) for channel in self.allowedIDsAMA]
