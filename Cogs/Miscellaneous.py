...
class Miscellaneous(commands.Cog):
    ...
    async def about(self, ctx):
        ...
        aboutEmbed.add_field(name="GitHub Link", value="COMING SOON", inline=True)
...
class Help(commands.HelpCommand):
    ...
    def channelCheck(self):
        if self.context.channel.id in self.allowedIDs:
            ...
            if len([channel for channel in textChannelAllowed if channel.guild.id == self.context.guild.id]) == 0:
    ...
    async def send_bot_help(self, mapping):
        ...
        if valid:
            ...
            valid, result = self.channelCheck()
            menu = PaginatedMenu(self.context)
            menu.set_destination(result)
